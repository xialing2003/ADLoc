...
if __name__ == "__main__":
    ...
    # with open(data_path + '/stations.json', 'r') as f:
    #     config_stations = json.load(f)
    # config_sta = pd.DataFrame(list(config_stations.items()), columns=['station_id', 'idx_sta'])
    # config_sta = config_sta[config_sta['idx_sta']!=-1]
    # config_sta['idx_sta'] = config_sta['idx_sta'] - 1

    # folder = data_path + '/picks'
    # file_names = os.listdir(folder)
    # num_event = len(file_names)
    ...
    # event_index = 0; eve_list = []
    # for file in file_names:
    #     eve_list.append(file.split('.')[0])

    #     picks = pd.read_csv(os.path.join(folder, file), parse_dates=["phase_time"])
    #     picks = picks.merge(config_sta, on="station_id")

    #     X = picks.merge(stations[["x_km", "y_km", "z_km", "idx_sta"]], on="idx_sta")
    #     t0 = X["phase_time"].min()
    ...
    num_event = len(events); event_input = np.ones((num_event,4))
    estimator = ADLoc(config, stations=stations[["x_km", "y_km", "z_km"]].values, num_event=num_event, eikonal=eikonal, events=event_input)

    for event_index in range(num_event):

        picks_event = picks[picks["event_index"] == event_index + 1]

        X = picks_event.merge(stations[["idx_sta"]], on="idx_sta")
        t0 = X["phase_time"].min()
        X.rename(columns={"phase_type": "type", "phase_time": "t_s"}, inplace=True)
        X["t_s"] = (X["t_s"] - t0).dt.total_seconds()
        X = X[["t_s", "type", "idx_sta"]]
        mapping_int = {"P": 0, "S": 1}
        X["type"] = X["type"].apply(lambda x: mapping_int[x.upper()])
    ...
    ev = estimator.events; ev[:,0:3] /= 2
    index = np.arange(num_event)

    # eve_list = np.array(eve_list).reshape(-1,1)
    save_eve = np.column_stack((index.astype(int),ev))
    np.savetxt(data_path + '/loc/reloc.txt', save_eve, fmt='%s')
    ...
    # plt.savefig(data_path + "/png/" + fold + "_xy.png")
    # plt.figure()
    ...
    # plt.savefig(data_path + "/png/" + fold + "_xz.png")
    # plt.figure()
    ...
    # plt.savefig(data_path + "/png/" + fold + "_yz.png")