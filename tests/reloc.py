...
#     # plt.show()
#     # plt.savefig("Init_loc_picks.png")
...
#         "mask": [0] * num_noise
#     }
#     noise = pd.DataFrame(noise_data)
#     merged_df = noise.merge(stations[["idx_sta", "x_km", "y_km", "z_km"]], on="idx_sta", how="left")
#     noise["x_km"] = merged_df["x_km"]
#     noise["y_km"] = merged_df["y_km"]
#     noise["z_km"] = merged_df["z_km"]
...
#     tt = estimator.predict(X[["idx_sta", "type"]].values, event_index=event_index)
#     print(f"True event loc: {event_loc[['x_km', 'y_km', 'z_km']].values}")
#     print(f"Estimated event loc: {estimator.events[event_index]}")
#     # %%
#     # plt.figure()
#     # X["dist_km"] = X[["x_km", "y_km", "z_km"]].apply(
#     #     lambda x: np.linalg.norm(x - event_loc[["x_km", "y_km", "z_km"]]), axis=1
...
# # %%
    ...
    for event_index in range(num_event):
        ...
        # tt = estimator.predict(X[["idx_sta", "type"]].values, event_index=event_index)
        # # print(f"True event loc: {event_loc[['x_km', 'y_km', 'z_km']].values}")
        # # print(f"Estimated {event_index} event loc: {estimator.events[event_index]}")

        est_loc.loc[event_index] = [estimator.events[event_index][0], estimator.events[event_index][1], estimator.events[event_index][2]]
        tru_loc.loc[event_index] = [event_loc['x_km'], event_loc['y_km'], event_loc['z_km']]
    
    ev = estimator.events
    print(ev[19][0],ev[19][1],ev[19][2])
    print(tru_loc.iloc[19])
    # index = np.arange(len(tru_loc))
    # plt.figure()
    # plt.scatter(tru_loc['x'], tru_loc['y'], c=index, cmap='hsv', marker='o')
    # plt.scatter(est_loc['x'], est_loc['y'], c=index, cmap='hsv', marker='x', s=100)
    # plt.xlabel('x_km'); plt.ylabel('y_km')
    # plt.savefig(data_path + "/error_xy.png")
    # plt.figure()
    # plt.scatter(tru_loc['x'], tru_loc['z'], c=index, cmap='hsv', marker='o')
    # plt.scatter(est_loc['x'], est_loc['z'], c=index, cmap='hsv', marker='x', s=100)
    # plt.xlabel('x_km'); plt.ylabel('z_km')
    # plt.savefig(data_path + "/error_xz.png")
    # plt.figure()
    # plt.scatter(tru_loc['y'], tru_loc['z'], c=index, cmap='hsv', marker='o')
    # plt.scatter(est_loc['y'], est_loc['z'], c=index, cmap='hsv', marker='x', s=100)
    # plt.xlabel('y_km'); plt.ylabel('z_km')
    # plt.savefig(data_path + "/error_yz.png")